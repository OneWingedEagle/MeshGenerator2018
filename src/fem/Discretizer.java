...
public class Discretizer {
	...
	private int nDimension=3;
	public int nBlocks,decimal,numberOfRegions,numberOfElements,numberOfNodes;
	private int maxMeshInOneDirection;

	public Discretizer(GUI gui){
		factor=1.0/gui.drp.scaleFactor;
		this.nBlocks=gui.drp.nBlocks;
		this.numberOfRegions=this.nBlocks+1;
		this.blockBoundary=new double[this.numberOfRegions][6];
		this.minMeshLeft=new double[this.numberOfRegions][6];
		this.minMeshRight=new double[this.numberOfRegions][6];
		this.baseLeft=new double[this.numberOfRegions][6];
		this.baseRight=new double[this.numberOfRegions][6];
		this.discLeft=new boolean[this.numberOfRegions][6];
		this.discRight=new boolean[this.numberOfRegions][6];

		for(int i=0;i<this.numberOfRegions;i++)
			for(int j=0;j<6;j++){
				this.blockBoundary[i][j]=gui.drp.blockBoundary[i][j];
				this.minMeshLeft[i][j]=gui.drp.minMeshLeft[i][j];
				this.minMeshRight[i][j]=gui.drp.minMeshRight[i][j];
				this.baseLeft[i][j]=gui.drp.baseLeft[i][j];
				this.baseRight[i][j]=gui.drp.baseRight[i][j];
				this.discLeft[i][j]=gui.drp.discLeft[i][j];
				this.discRight[i][j]=gui.drp.discRight[i][j];
			}	
	...
	private void setXYZ(){
		...
		double maxDimension=max(max(this.blockBoundary[0][1]-this.blockBoundary[0][0],
		...
		double[][] XYZ=new double[this.maxMeshInOneDirection][this.nDimension];
		double[] v;

		double d1=1e10,d2=d1,b1,b2;
		int N= this.sortedBoundary.length;
		int blockLeft=0,blockRight=0;
		int[] nXYZ=new int[this.nDimension];
		int j1,j2;
		for(int j=0;j<this.nDimension;j++){
			...
			for(int i=0;i<N-1;i++) {
				...
				if(this.sortedBoundary[i][j]==this.sortedBoundary[i+1][j]) XYZ[nXYZ[j]++][j]=this.sortedBoundary[i+1][j];
				else{
					if(i==0){
						b1=0;
						blockLeft=0;
		...
		double[] tempX=new double[nXYZ[0]],	tempY=new double[nXYZ[1]],tempZ=new double[nXYZ[2]];
		tempX[0]=XYZ[0][0];
		tempY[0]=XYZ[0][1];
		tempZ[0]=XYZ[0][2];
		int ix=1,iy=1,iz=1;
	...
	public void sortBoundary(){
		...
		for(int j=0;j<J;j++){
			for(int i=0;i<I-2;i++){
				if(i%2==0)  v.el[i]=this.blockBoundary[i/2+1][2*j];
				else
					v.el[i]=this.blockBoundary[(i-1)/2+1][2*j+1];

			}
			int[] indice=v.bubble();

			for(int i=0;i<I-2;i++){				
				this.sortedBoundary[i+1][j]=v.el[i];
				this.sortedIndex[i+1][j]=indice[i]+2;

			}
			this.sortedBoundary[0][j]=this.blockBoundary[0][2*j];
			this.sortedBoundary[I-1][j]=this.blockBoundary[0][2*j+1];
			this.sortedIndex[0][j]=0;
			this.sortedIndex[I-1][j]=1;
		}		
